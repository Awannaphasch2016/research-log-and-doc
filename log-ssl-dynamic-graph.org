...
1. original
2. with positive edges weighted as ef_iwf (--ef_iwf_window_dict)
3. with positive and negative edges weighted as user nf_iwf (--use_ef-iwf_weight)
...
class crawler:
  def __init__(self, log_file):
    ...
    self.nodes_and_edges_wight_log_path = str(base_path / f'log/nodes_and_edges_weight/{log_file}.log')
    self.plot_path = str(base_path / f'plot/{log_file}.png')
  ...
  def crawl_data_v2(self, log_path):
    ...
    with open(log_path, 'r') as f:
      for i, line in enumerate(f.readlines()): # 9341 lines in total
        ...
        if is_comma_exist and not is_neg_edges_weight:
      ...
      pos_edges_weight = pos_edges_weight[-1, -max(self.ws)+1:, :]
...
- run 5 times the model with sigmoid ef-iwf loss (set =use_sigmoid_ef_iwf_weight= flag to True) + first 10,000 reddit instances
